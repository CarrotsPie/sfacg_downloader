...
def get_sign(nonce, timestamp, device_token):
    ...
    sign = hashlib.md5(final.encode("utf-8")).hexdigest().upper()
...
def download_chapter(chapters):
    ...
    for chapter in chapters:
        try:
            resp = {"status":{"httpCode":417}}
            t = 0
            while (resp['status']['httpCode']==417 and t<5):
                t = t + 1
                content = ""
...
def check(headers):
    resp = requests.get('https://api.sfacg.com/user?', headers=headers)
    data = resp.json()
    try:
...
if __name__ == "__main__":
    ...
    if not os.path.exists(cookie_file):
        ...
        with open(cookie_file, "w", encoding="utf-8") as f:
            headers['cookie'] = ""  # 这里可以填默认cookie
            f.write(headers['cookie'])
        print("cookie.txt 已创建")
    else:
        # 文件存在，读取内容
        with open(cookie_file, "r", encoding="utf-8") as f:
            headers['cookie'] = f.read().strip()
        print("读取到 cookie:", headers['cookie'])
    while (check(headers)):
